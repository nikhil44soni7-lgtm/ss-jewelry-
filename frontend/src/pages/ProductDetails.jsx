...
export const ProductDetails = ({ productId }) => {
  ...
  return (
    <div className={productId ? "bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 font-sans" : "bg-slate-55 dark:bg-slate-950 text-slate-800 dark:text-slate-100 min-h-screen pb-6 font-sans"}>
      ...
      {/* Sticky Header Bar */}
      {!productId && (
        <div className={`sticky top-[64px] bg-white/95 dark:bg-slate-900/95 backdrop-blur-md z-[35] border-b border-slate-200/80 dark:border-slate-800/80 py-3 mb-4 w-full shadow-sm ${isAdmin && !isPreviewMode ? 'hidden md:block' : ''}`}>
      ...
      <div className={`w-full ${(isAdmin && !isPreviewMode) ? 'max-w-[1700px]' : 'max-w-[1600px]'} mx-auto px-4 md:px-6`}>
        ...
        {isAdmin && !isPreviewMode ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[40%_60%] gap-4 lg:gap-10 items-start w-full">
            {/* LEFT COLUMN: Gallery & Trust Section (40%) */}
            <div className="space-y-6 w-full">
              {/* Product Gallery Container */}
              <div className="flex flex-col gap-3 md:gap-4 select-none w-full">
                {/* Main Large Image Card with dynamic height */}
                <div className="w-full bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-4 md:p-5 relative group z-30 flex items-center justify-center">
                  <div 
                    className="relative w-full h-auto min-h-[220px] sm:min-h-[280px] md:min-h-[350px] lg:min-h-[400px] flex items-center justify-center overflow-hidden rounded-xl cursor-zoom-in"
                      ...
                      <button
                        ...
                        className={`w-16 h-16 lg:w-20 lg:h-20 rounded-xl border-2 overflow-hidden flex-shrink-0 transition-all cursor-pointer ${
                          (activeImage || imagesList[0]) === img
                            ? 'border-emerald-500 dark:border-[#D4A75F] ring-2 ring-emerald-500/10 dark:ring-[#D4A75F]/30 shadow-md scale-105'
                    ...
                    {product.discount > 0 && (
                      <>
                        <span className="text-base text-slate-400 line-through font-medium">
                          {language === 'hi' ? 'एम.आर.पी' : 'M.R.P'}: ₹{formatPrice(product.price)}
                        </span>
                ...
                <div className="divide-y divide-slate-100 dark:divide-slate-800/80 text-xs px-2">
                  ...
                  <div className="py-3 flex items-start gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="text-slate-500 dark:text-slate-400 mt-0.5">
                        {language === 'hi' ? (
                          <>₹{formatPrice(Math.floor(discountedPrice / 12))}/माह से शुरू होने वाले ब्याज मुक्त विकल्प</>
                        ) : (
                          <>Interest-free options starting from ₹{formatPrice(Math.floor(discountedPrice / 12))}/month</>
                        )}
                      </p>
                    </div>
                  </div>
